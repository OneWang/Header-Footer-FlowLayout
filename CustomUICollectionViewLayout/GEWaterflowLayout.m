...
@implementation GEWaterflowLayout
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.leftMargin = 10;
        self.rightMargin = 10;
        self.sectionEdgeInset = UIEdgeInsetsMake(10, 0, 10, 0);
        self.columnsCount = 3;
        self.section = 0;
    }
    return self;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.addItemByOrder == YES) {   // 顺序添加, 不管某一行的高度有多高
        minColumn = [NSString stringWithFormat:@"%zd", indexPath.item % self.columnsCount];
    } else {    // 找出最短的那一列
    ...
    // 计算尺寸
    CGFloat width = (self.collectionView.frame.size.width - self.leftMargin - self.rightMargin - (self.columnsCount - 1) * self.columnMargin)/self.columnsCount;
    CGFloat height = [self.delegate waterflowLayout:self heightForWidth:width atIndexPath:indexPath];
    
    // 计算位置
    CGFloat x = self.leftMargin + (width + self.columnMargin) * [minColumn intValue];
...
- (UICollectionViewLayoutAttributes *)attributesForHeaderAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.section != indexPath.section) {
        attrs.frame = CGRectMake(x, maxY + self.sectionEdgeInset.top, headerSize.width, headerSize.height);
        // 将下一组的初始 Y 值更新
...
- (UICollectionViewLayoutAttributes *)attributesForFooterAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat maxY = [self getMaxY];
    attrs.frame = CGRectMake(x, maxY + self.sectionEdgeInset.bottom, footerSize.width, footerSize.height);
